...
def run(srcScreen,name, duration = 3.0, x = -1, y = -1):
...
def updateScreen():
    ...
    if transition != False:
        ...
        if time_ratio > 1.0:
            ...
            if transition == "fade":
                transition_data.current_screen.set_alpha(255*time_ratio)
                new = transition_data.screen.copy()
                new.blit(transition_data.current_screen,[0,0])
                screen.blit(new,[0,0])
            else:    
